...
def _build_docx(docx_path, patient_name, patient_dob, prescriptions, today,
    ...
    # Colonne gauche: signature image puis nom
    _clear_cell(cell_left)
    if SIG_PATH.exists():
        sig_p = cell_left.add_paragraph()
        sig_p.alignment = WD_ALIGN_PARAGRAPH.LEFT
        sig_run = sig_p.add_run()
        sig_run.add_picture(str(SIG_PATH), width=Cm(4.0))
        # Remonter la signature avec espacement negatif
    ...
    # Colonne droite: vide (zone pour tampon eventuel)
