...
def _build_docx(docx_path, patient_name, patient_dob, prescriptions, today,
    ...
    # Bloc bas gauche: signature en superposition sur nom medecin
    # Utiliser un tableau 2 colonnes: gauche=signature+nom, droite=vide
    bottom_tbl = doc.add_table(rows=1, cols=2)
    bottom_tbl.style = "Table Grid"
    # Supprimer les bordures
    from docx.oxml import OxmlElement
    from docx.oxml.ns import qn
    for row in bottom_tbl.rows:
    ...
    cell_left = bottom_tbl.rows[0].cells[0]
    cell_right = bottom_tbl.rows[0].cells[1]

    # Colonne gauche: NOM puis SIGNATURE superposee par-dessus
    _clear_cell(cell_left)

    # 1. Texte nom + specialite
    dr_p = cell_left.add_paragraph()
    dr_p.alignment = WD_ALIGN_PARAGRAPH.LEFT
    dr_p.paragraph_format.space_before = Pt(0)
    dr_p.paragraph_format.space_after = Pt(0)
    dr_r = dr_p.add_run("Docteur " + doctor_name + "\n" + doctor_specialty)
    dr_r.font.size = Pt(10)
    dr_r.font.name = "Arial"

    # 2. Signature: meme paragraphe avec position negative pour superposition
    if SIG_PATH.exists():
        sig_p = cell_left.add_paragraph()
        ...
        sig_run.add_picture(str(SIG_PATH), width=Cm(4.2))
        # Position verticale negative: remonte la signature sur le texte
        rPr = sig_run._r.get_or_add_rPr()
        pos_el = OxmlElement("w:position")
        pos_el.set(qn("w:val"), "-48")
        rPr.append(pos_el)
